...
export default function ClientDetails() {
    ...
    const handleGenerateSummary = async () => {
        ...
        try {
            const res = await api.get(`/api/psychologist/client/${id}/summary`);
            setAiSummary(res.data);
            toast.success(t('client_details_summary_success'));
        } catch (e) { 
            toast.error(t('client_details_summary_error')); 
        } finally { 
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 'var(--space-xl)', flexWrap: 'wrap', gap: 'var(--space-md)' }}>
                ...
                <div className="btn-row">
                    <button onClick={handleGenerateSummary} className="btn-secondary" disabled={generatingSummary}>
                        {generatingSummary ? t('client_details_generating') : t('client_details_ai_summary')}
                    </button>
            ...
            {aiSummary && (
                <div className="card" style={{ marginBottom: 'var(--space-xl)', borderLeft: '4px solid var(--accent-primary)', background: 'var(--accent-primary-glow)' }}>
                    <h3 style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--accent-primary)' }}>
                        {t('client_details_ai_clinical_title')}
                    </h3>
                    ...
                    <div key={rec.id} className="card">
                        ...
                        {(rec.aiAnalysis || rec.interpretation) && (
                            <div style={{ background: 'var(--bg-surface-2)', padding: '15px', marginTop: '15px', borderRadius: 'var(--radius-md)', borderLeft: '4px solid var(--accent-primary)' }}>
                                <div className="input-label" style={{ color: 'var(--accent-primary)' }}>{t('client_details_ai_analysis')}</div>
