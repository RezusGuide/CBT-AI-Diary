...
export default function PsychologistNotes() {
    ...
    const fetchNotes = async () => {
        try {
            const res = await api.get(`/api/notes?psychologistId=${userId}`);
            setNotes(res.data);
        } catch (e) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await api.post('/api/notes', { ...formData, psychologistId: userId });
            setIsCreating(false);
                    ...
                    <div key={note.id} className="card">
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 'var(--space-md)' }}>
                            <div>
                                ...
                                <div className="badge badge-violet" style={{ marginTop: '6px' }}>{t('chat_client')}: {note.client?.fullName || '—'}</div>
                            </div>
